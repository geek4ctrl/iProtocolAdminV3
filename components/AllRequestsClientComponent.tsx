...
export default function AllRequestsClientComponent({ allRequests }: { allRequests: any }) {
    ...
    const allRequestsAfterFilter = {
        nodes: allRequests.filter((item: any) =>
    ...
    const currentRecords = allRequestsAfterFilter.nodes.slice(indexOfFirstRecord, indexOfLastRecord);

    const nPages = Math.ceil(allRequestsAfterFilter.nodes.length / recordPerPage);
    ...
    const selectedItem = useStore((state: any) => state.navigationState);
